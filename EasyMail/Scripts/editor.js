class Editor {
	...
	clearErrors(errors) {
		errors.forEach(error => {
			Object.defineProperties(error, {
				...
				range: {
					startColumn: error.range.startColumn - 2,
					endColumn: error.range.endColumn + 2
				}
			})
		})
		this.editor.deltaDecorations([], errors)
	}
	...
	repositionSuggestion() {
		...
		var isLarger = (parseInt(popupWidth) + pos.left) > winWidth + 200;
		if (isLarger) pos.left = winWidth - (parseInt(popupWidth) - 250);
